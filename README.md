...
> **이 브랜치(`main`)는 세로 전용이다.**
>
> 아이패드 세로(820&times;1180)를 전제로 만들었다. 가로로 돌리면 화면 아래가 잘려
> 시작 화면의 "화면을 눌러 시작하기", 안내 화면의 "촬영 시작", 번호 입력 화면의
> "확인" 버튼을 누를 수 없다.
>
> 세로·가로 양쪽에서 쓰려면 [`feat/landscape-layout`](../../tree/feat/landscape-layout)
> 브랜치를 본다. 그 브랜치 README 의 "가로 대응" 절에 바뀐 내용이 정리되어 있다.
