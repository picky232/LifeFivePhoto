...
API 명세의 각 항목을 자동으로 검사한다. 서버를 별도 포트(3443)와 임시 저장 경로로 직접 띄우므로, 운영 중인 서버가 3000번에서 돌고 있어도 함께 실행할 수 있고 실제 `output/` 폴더도 건드리지 않는다. 기동부터 정리까지 자동으로 처리하며, 실패가 하나라도 있으면 종료 코드 1을 반환한다.

인증서가 없으면 실행되지 않는다. 먼저 `node generate-cert.js`를 실행한다.
