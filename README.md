...
> **이 브랜치는 세로·가로 양쪽 대응 버전이다.**
>
> `main` 은 세로 전용이다. 아이패드를 가로로 돌리면 화면 아래가 잘려 버튼을 누를 수 없다.
> 이 브랜치는 두 방향 모두에서 잘리지 않도록 고쳤다. 바뀐 내용은 [가로 대응](#가로-대응) 참고.
>
> 어느 쪽을 쓸지는 부스를 어느 방향으로 세울지에 달렸다. 세로로 세운다면 `main` 으로 충분하다.
...
칸 높이 560 중 아래 74px 은 학과 이름표 자리다. 이름표를 프레임에 그려 넣으면 그 부분은 불투명하게 두고 위쪽만 뚫으면 된다.
...
`main` 은 세로(820&times;1180)를 전제로 만들어졌다. 가로는 높이가 820px 뿐이라 세로용 크기를 그대로 쓰면 화면 아래가 잘린다. 이 브랜치는 그 문제와, 고치는 과정에서 발견한 것들을 함께 정리했다.
