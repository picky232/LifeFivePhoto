...
칸 높이 560 중 아래 73px 은 학과 이름표 자리다. 이름표를 프레임에 그려 넣으면 그 부분은 불투명하게 두고 위쪽 487px 만 뚫으면 된다.
...
촬영 함수는 카메라 영상을 컷 비율(549&times;560, 거의 정사각형)로 가운데만 잘라낸다. 그런데 미리보기 상자는 `w-full` 때문에 가로로 늘어난 직사각형이었다. **화면에서 보이던 좌우가 저장할 때 잘려 나갔다.** 가로에서 특히 심했다.
