...
준비가 끝난 뒤에는 `부스-시작.bat` 을 두 번 누르는 것으로 끝난다. 핫스팟을 켜고 접속 정보를 보여준 뒤 서버를 띄운다. 창을 닫으면 서버도 멈춘다.

| 파일 | 하는 일 |
|---|---|
| `부스-시작.bat` | 핫스팟 켜기 + 서버 실행 (평소에는 이것만 쓴다) |
| `핫스팟-켜기.bat` | 핫스팟만 켠다 |
| `핫스팟-끄기.bat` | 핫스팟만 끈다 |
| `서버만-실행.bat` | 서버만 띄운다 (핫스팟이 이미 켜져 있을 때) |

`부스-시작.bat` 은 실행 전에 아래를 먼저 확인하고, 빠진 것이 있으면 알아서 채운다.
